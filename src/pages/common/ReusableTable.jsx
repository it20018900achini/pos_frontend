...
const ReusableTable = ({
  columns = [], // each column: { header, accessor, total?, sortable? }
  data = [],
  loading = false,
  actions = null,
  isClient = false,
  ...
  fetchAllData = null,
  isNumbering = true,
  view = "table", // new prop: "table" | "card" | "list"
}) => {
      ...
      <TableBody>
        ...
        {Object.keys(totals).length > 0 && (
          <TableRow className="font-bold bg-gray-100">
            ...
            {columns.map((col) => (
              <TableCell key={col.accessor}>{col.total ? totals[col.accessor] : ""}</TableCell>
            ))}
            {actions && <TableCell />}
          </TableRow>
        )}
