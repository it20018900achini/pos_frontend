...
export default function DashboardLayout() {
  ...
  const { userProfile, selectedBranchId, loading } = useSelector((state) => state.user);
  ...
  useEffect(() => {
    ...
    if (selectedBranchId) {
      dispatch(getBranchById({ id: selectedBranchId, jwt: token }));
    }

    if (userProfile.user.storeId) {
      dispatch(getStoreByAdmin(token));
      dispatch(getAllBranchesByStore({ storeId: userProfile.user.storeId, jwt: token }));
    }
  }, [dispatch, userProfile, selectedBranchId]);
    ...
    return (
      <div className="flex items-center justify-center h-screen">
        <p className="text-lg text-gray-500 text-center">
          Unauthorized Access
          <br />
          <Link to="/auth/login" className="text-blue-500 hover:underline">Go to Login</Link>
        </p>
      </div>
    );
  }
